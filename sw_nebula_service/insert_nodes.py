...
def insert_directory_nodes(engine: NebulaEngine, name_space: str):
    ...
    for i, lib_name in enumerate(DIR_STRUCTURE.keys()):
        lib_vid = engine.insert_node(name_space=name_space, node=LibNode(name=f"lib_{lib_name}"), vid=i)
        ...
        for j, ontology_name in enumerate(DIR_STRUCTURE[lib_name]):
            ontology_vid = engine.insert_node(name_space=name_space, node=OntologyNode(name=f"onto_{ontology_name}"), vid=i + j)
